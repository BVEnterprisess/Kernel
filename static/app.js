...
const state = { health:null, stats:null, reality:null, settings:null, defaultNamespace:'', selectedNamespace:'__all__', view:'overview', memoryOffset:0, selectedMemory:null, recentWrites:[], identityScope:null, actorIdentityFilter:'__all__' };
const VALID_VIEWS = new Set(['overview','reality','visualiser','memories','recall','skills','conflicts','graph','identity-scope','lifecycle','ops','settings']);
...
function setView(view, opts={}){
  ...
  const names={overview:'Memory Operations',reality:'Operational Reality & Action Space',visualiser:'Visualiser',memories:'Memory Browser',recall:'Recall',skills:'Learned Skills',conflicts:'Contradictions',graph:'Entity Graph','identity-scope':'Identity & Scope',lifecycle:'Lifecycle',ops:'Maintenance',settings:'Settings'};
...
async function init(){
  ...
  $('#refreshRealityBtn')?.addEventListener('click',()=>loadReality());
...
async function loadStats(){
  try {
    const [statsData, realityData] = await Promise.all([
      api(`/api/stats?namespace=${encodeURIComponent(ns())}`),
      api('/api/reality').catch(err => { console.warn('Reality endpoint unavailable', err); return null; })
    ]);
    state.stats = statsData;
    if(realityData) {
      state.reality = realityData;
      updateRealityTopBar(realityData);
    }
  } catch(e) {
  ...
  const currentSeq = state.reality?.lastObservedSeq ?? state.stats?.cluster_seq ?? 0;
  const metrics=[
    ['Active',active],
    ['Consolidated',consolidated],
    ['Forgotten',tomb],
    ['Cluster Seq', `#${currentSeq}`],
    ['Open conflicts',state.stats.open_conflicts],
    ['Entities',state.stats.entities],
    ['Edges',state.stats.edges],
    ['DB size',humanBytes(state.health.db_size_bytes)]
  ];
  ...
  renderOverviewAuditTrail();
  updateScopeUi();
}

function updateRealityTopBar(reality){
  if(!reality) return;
  const seqBadge = $('#clusterSeqBadge');
  if(seqBadge) {
    seqBadge.textContent = `Cluster Seq: ${reality.lastObservedSeq ?? 0}`;
    seqBadge.title = `Monotonically advancing cluster sequence: ${reality.lastObservedSeq ?? 0}`;
  }
  const modeBadge = $('#backendModeBadge');
  if(modeBadge) {
    modeBadge.textContent = `Mode: ${reality.backendMode || 'embedded'}`;
  }
}

function renderOverviewAuditTrail(){
  const container = $('#overviewAuditTrail');
  if(!container) return;
  const mutations = state.reality?.latestMutations || [];
  const seq = state.reality?.lastObservedSeq ?? 0;
  const seqPill = $('#overviewAuditSeq');
  if(seqPill) seqPill.textContent = `Observed Seq: ${seq}`;

  if(!mutations.length){
    container.innerHTML = `<div class="p-4 text-xs text-zinc-400">No mutations recorded in session yet. Monotonic cluster sequence is ${seq}. Perform a memory tombstone or maintenance pass to observe real mutation causality.</div>`;
    return;
  }

  container.innerHTML = `<table class="reality-audit-table">
    <thead>
      <tr>
        <th>Seq</th>
        <th>Timestamp</th>
    ...
    <tbody>
      ${mutations.slice(0, 5).map(m => `
        <tr>
          <td><span class="pill seq-pill">#${m.sequenceAdvancement?.to ?? m.resultingSeq ?? '—'}</span></td>
          <td class="font-mono text-zinc-400">${new Date(m.timestamp * 1000).toLocaleTimeString()}</td>
          <td><strong class="text-white">${esc(m.actionName || m.actionId)}</strong></td>
          <td><span class="pill epistemic-${(m.epistemic || 'observed').toLowerCase()}">${esc(m.epistemic || 'OBSERVED')}</span></td>
          <td class="break-anywhere">${esc(m.outcomeSummary || 'Executed successfully')}</td>
        </tr>
      `).join('')}
    </tbody>
  </table>`;
...
function memoryItem(m){
  ...
  const epistemic = m.epistemic_status || 'OBSERVED';
  const decayPct = m.temporal_decay_factor !== undefined ? Math.round(m.temporal_decay_factor * 100) : null;
  return `<div class="memory-item" data-rid="${esc(m.rid)}">
    <div class="memory-headline">
      <div class="memory-title">${esc(m.domain||'general')} <span class="muted">· ${esc(m.type)}</span></div>
      <div class="flex items-center gap-1.5">
        <span class="pill epistemic-${esc(epistemic.toLowerCase())}" title="Epistemic status: ${esc(epistemic)}">${esc(epistemic)}</span>
        ${showNs?`<span class="namespace-chip" title="${esc(m.namespace||'')}">${esc(shortNamespace(m.namespace))}</span>`:''}
      </div>
    </div>
    <div class="memory-text">${esc(m.text)}</div>
    <div class="meta-row">
      <span class="pill ${statusPillClass(m.consolidation_status)}">${esc(statusLabel(m.consolidation_status))}</span>
      <span class="pill">importance ${Number(m.importance||0).toFixed(2)}</span>
      ${decayPct !== null ? `<span class="pill neutral" title="Ranking decay factor computed against event time">decay ${decayPct}%</span>` : ''}
      <span class="pill">${esc(m.source||'source')}</span>
      <span class="pill">${esc(m.created_at_iso||'')}</span>
    </div>
  </div>`;
}
...
function detailHtml(m){
  ...
  return `<section class="memory-detail-shell">
    ...
    <div class="diag-grid">
      ${diag('RID', rid, true)}${diag('Epistemic Status', m.epistemic_status || 'OBSERVED')}${m.temporal_decay_factor !== undefined ? diag('Ranking Decay', `${Math.round(m.temporal_decay_factor*100)}%`) : ''}${diag('Namespace', m.namespace, true)}${diag('Source', m.source)}${diag('Type', m.type)}${diag('Importance', Number(m.importance||0).toFixed(3))}${diag('Certainty', Number(m.certainty||0).toFixed(3))}${diag('Access count', m.access_count)}${diag('Created', m.created_at_iso)}${diag('Updated', m.updated_at_iso)}${m.event_time?diag('Event time', new Date(m.event_time*1000).toISOString()):''}${diag('Embedding bytes', m.embedding_bytes)}
    </div>
...
async function runRecall(){
  ...
  $('#recallResults').innerHTML=`<div class="panel"><div class="panel-head"><h2>Results</h2><span class="muted">${fmt(results.length)} hits</span></div>${(data.certainty_reasons||[]).map(x=>`<span class="pill warn">${esc(x)}</span>`).join('')}</div>`+results.map((r,i)=>`<div class="recall-card"><div class="panel-head"><h2>#${i+1} ${esc(r.domain||r.type||'memory')}</h2><div class="recall-score">${Number(r.score||r.similarity||0).toFixed(3)}</div></div><p>${esc(r.text||r.content||JSON.stringify(r).slice(0,500))}</p>${r.fallback?`<div class="recall-fallback-banner"><span class="pill warn">⚠️ Degraded Recall Mode</span> <span>This result was retrieved via <strong>${esc(r.fallback)}</strong> fallback because semantic vector lane did not meet threshold.</span></div>`:''}${renderScoreBreakdown(r.scores||{})}<div class="meta-row">${(r.why_retrieved||r.reasons||[]).map(x=>`<span class="pill good">${esc(x)}</span>`).join('')}<span class="pill">${esc(r.rid||'')}</span>${r.event_time_start?`<span class="pill neutral">Event: ${new Date(r.event_time_start*1000).toISOString().slice(0,10)}</span>`:''}</div><details class="details-block"><summary>Raw recall row</summary><pre>${esc(JSON.stringify(r,null,2))}</pre></details></div>`).join('') || `<div class="empty">No recall results.</div>`;
}
...
async function loadReality(){
  try {
    const reality = await api('/api/reality');
    state.reality = reality;
    updateRealityTopBar(reality);
    renderRealityView(reality);
  } catch(e) {
    toast(`Could not load reality projection: ${e.message}`);
  }
}

function renderRealityView(reality){
  if(!reality) return;
  const backendPill = $('#realityBackendPill');
  if(backendPill) {
    backendPill.textContent = `Mode: ${reality.backendMode || 'embedded'}`;
    backendPill.className = reality.backendMode === 'embedded' ? 'pill good' : 'pill hot';
  }
  const seqPill = $('#realitySeqPill');
  if(seqPill) seqPill.textContent = `Observed Sequence: #${reality.lastObservedSeq ?? 0}`;
  
  const adminPill = $('#realityAdminPill');
  if(adminPill) {
    const isAdmin = !!(state.health?.admin_enabled || state.settings?.admin_mode);
    adminPill.textContent = isAdmin ? 'Admin: Authorized' : 'Admin: Read-Only';
    adminPill.className = isAdmin ? 'pill warn' : 'pill neutral';
  }

  const healthPill = $('#realityHealthPill');
  if(healthPill) {
    healthPill.textContent = `Health: ${(reality.systemHealth?.status || 'ok').toUpperCase()}`;
    healthPill.className = reality.systemHealth?.status === 'ok' ? 'pill good' : 'pill warn';
  }

  // Action space rendering
  const actionGrid = $('#realityActionSpaceGrid');
  if(actionGrid) {
    const actions = reality.actionSpace || [];
    actionGrid.innerHTML = actions.map(act => {
      let statusBadge = '';
      let cardClass = 'action-capability-card';
      if(!act.available) {
        statusBadge = `<span class="pill epistemic-unsupported">UNAVAILABLE</span>`;
        cardClass += ' unavailable';
      } else if(!act.authorized) {
        statusBadge = `<span class="pill epistemic-uncertain">UNAUTHORIZED (Admin Mode Required)</span>`;
        cardClass += ' unauthorized';
      } else {
        statusBadge = `<span class="pill epistemic-observed">AUTHORIZED & AVAILABLE</span>`;
      }

      return `<div class="${cardClass}">
        <div class="flex items-center justify-between gap-2 mb-2">
          <strong class="text-white text-sm">${esc(act.name)}</strong>
          ${statusBadge}
        </div>
        <p class="text-xs text-zinc-400 mb-3">${esc(act.description)}</p>
        <div class="flex flex-wrap gap-1.5 text-[11px] font-mono">
          <span class="pill neutral">${esc(act.method)} ${esc(act.endpoint)}</span>
          <span class="pill neutral">Required: ${esc(act.capabilityRequired)}</span>
          ${act.requiresAdmin ? '<span class="pill warn">Requires Admin</span>' : ''}
        </div>
      </div>`;
    }).join('') || '<div class="empty">No action space definitions available.</div>';
  }

  // Full causal mutation audit table
  const mutationCountPill = $('#realityMutationCount');
  if(mutationCountPill) {
    mutationCountPill.textContent = `${reality.mutationsCount || 0} mutation${reality.mutationsCount === 1 ? '' : 's'} recorded`;
  }

  const auditContainer = $('#realityAuditTable');
  if(auditContainer) {
    const trail = reality.latestMutations || [];
    if(!trail.length) {
      auditContainer.innerHTML = `<div class="p-5 text-sm text-zinc-400 text-center">No mutations recorded in session yet. Monotonic cluster sequence is currently #${reality.lastObservedSeq ?? 0}.</div>`;
    } else {
      auditContainer.innerHTML = `<table class="reality-audit-table">
        <thead>
          <tr>
            <th>Seq Advancement</th>
            <th>Event Time</th>
            <th>Operation</th>
            <th>Authority</th>
            <th>Epistemic Status</th>
            <th>Affected Records</th>
            <th>Causal Outcome</th>
          </tr>
        </thead>
        <tbody>
          ${trail.map(entry => `
            <tr>
              <td>
                <span class="pill seq-pill">#${entry.sequenceAdvancement?.from ?? '—'} &rarr; #${entry.sequenceAdvancement?.to ?? entry.resultingSeq ?? '—'}</span>
              </td>
              <td class="font-mono text-zinc-400">${new Date(entry.timestamp * 1000).toLocaleTimeString()}</td>
              <td><strong class="text-white">${esc(entry.actionName || entry.actionId)}</strong></td>
              <td><span class="pill ${entry.authority === 'admin' ? 'warn' : 'neutral'}">${esc(entry.authority)}</span></td>
              <td><span class="pill epistemic-${(entry.epistemic || 'observed').toLowerCase()}">${esc(entry.epistemic || 'OBSERVED')}</span></td>
              <td class="font-mono text-zinc-300">${(entry.affectedRids || []).map(r => `<code>${esc(r)}</code>`).join(', ') || '—'}</td>
              <td class="break-anywhere text-zinc-200">${esc(entry.outcomeSummary || 'Done')}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>`;
